...
public class EMP_breakdown implements RegionalizationMethod {
    ...
    public static RegionCollection construction_phase_breakdown(ArrayList<Integer> idList,
        ...
        Map<Integer, Region> regionList_MaxP_AVG = null;
        int[] labels_MaxP_AVG = minmaxlabels.clone();
        int maxp_avg = 0;
        for (int it = 0; it < maxIt; it++) {
            ...
            labels = avgInitialization.getValue0().clone();
            ...
            //Test: Select the initialization result with the max p after the AVG step
            if (regionList.size() > maxp_avg){
                regionList_MaxP_AVG = new HashMap<>(regionList);
                maxp_avg = regionList.size();
                labels_MaxP_AVG = labels.clone();
        ...
        if (check_p_afterAVG) {
            System.out.println("-------------");
            System.out.println("P after AVG: " + regionList_MaxP_AVG.size());
            System.out.println("-------------");
        }
        if(labelCheck){
            System.out.println("Label check after AVG");
            checkLabels(labels_MaxP_AVG, regionList_MaxP_AVG);
        }

        for (int it = 0; it < maxIt; it++) {
            int[] initlabels = labels_MaxP_AVG.clone();
            Map<Integer, Region> initregionList = (HashMap<Integer, Region>)((HashMap<Integer, Region>) regionList_MaxP_AVG).clone();
            //regionList = (HashMap<Integer, Region>)((HashMap<Integer, Region>) regionList_MaxP_AVG).clone();
            if(labelCheck){
                System.out.println("Label check before sum");
                checkLabels(initlabels, initregionList);
            }
            ArrayList<Integer> seedAreas = new ArrayList<>(minmaxseedAreas);
            Pair<int[], Map<Integer, Region>> result = sumcount_construction(initlabels, initregionList, r, minAttr, maxAttr, avgAttr, sumAttr, sumLowerBound, sumUpperBound, countLowerBound, countUpperBound);
            int[] labels = result.getValue0();
            Map<Integer, Region> regionList = result.getValue1();
